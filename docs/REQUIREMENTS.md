...
- ✅ **Expired service patterns are hidden from riders** (#71). A pattern whose `calendar.txt` range ended before the agency's today is dropped from the tab row, with a quiet "Past service patterns are hidden. Show all" note and a `?show_expired=1` link that brings it back marked "(ended)". Exceptions, all deliberate: an explicit `?service=<id>` at an expired pattern still renders (someone has that URL pinned), with a red "This schedule ended on …" banner — suppressed when the feed-level expiry banner is already saying the broader thing; when *every* pattern has expired they are all shown, marked, with the banner; the auto-selected default never lands on an expired pattern while a live one exists; and a pattern a `calendar_dates.txt` `exception_type=1` row revives on or after today is not expired at all. `/api/v1/services` deliberately does NOT hide them — it adds `expired` + `end_date` and the Embed panel's picker lists them marked, because an agency may want to pin a seasonal pattern before its season.
...
- ✅ **Service-pattern picker** in the Embed tab — pins `?service=<profile_id>` into the copied per-route snippets so an embed always shows a chosen schedule (Saturday, a seasonal pattern) instead of today's. Options come from the published snapshot via `feeds.*/<slug>/api/v1/services`, the same `buildServiceProfiles` the embed pages resolve `?service=` with (`shared/serviceProfiles.ts`, imported by both the Worker and the SPA). The pin is emitted only on routes that actually run the pattern, and never on the system map, which has no service-day concept.
